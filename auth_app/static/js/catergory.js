...
async function loadEbooks(query = 'fantasy') {
  ...
  try {
    ...
    if (data.docs && data.docs.length > 0) {
      const books = shuffleArray(data.docs).slice(0, 20);
      ...
      books.forEach(book => {
        ...
        bookItem.innerHTML = `
          <div class="book-cover-wrap">
            ...
            <div class="book-card-buttons" data-key="${key}" data-title="${title.replace(/"/g, '&quot;')}" data-author="${author.replace(/"/g, '&quot;')}">
...
// ============================================
// BOOKMARK / FAVOURITE BUTTONS
// ============================================
function addBookButtons() {
  document.querySelectorAll('.book-card-buttons').forEach(container => {
    ...
    if (bookmarkBtn) {
      bookmarkBtn.addEventListener('click', async (e) => {
        ...
        e.stopPropagation();
        try {
          const res = await fetch(`/accounts/toggle-bookmark/${cleanKey}/`, {
            method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' }
          });
          const data = await res.json();
          if (data.status === 'added') bookmarkBtn.classList.add('is-active');
          else if (data.status === 'removed') bookmarkBtn.classList.remove('is-active');
    ...
    if (favoriteBtn) {
      favoriteBtn.addEventListener('click', async (e) => {
        ...
        e.stopPropagation();
        try {
          const res = await fetch(`/accounts/toggle-favourite/${cleanKey}/`, {
            method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' }
          });
          const data = await res.json();
          if (data.status === 'added') favoriteBtn.classList.add('is-active');
          else if (data.status === 'removed') favoriteBtn.classList.remove('is-active');
          else alert(data.error || 'Something went wrong');
        } catch (error) {
...
function addViewTracking() {
  document.querySelectorAll('.view-book-link').forEach(link => {
    link.addEventListener('click', () => {
      const key = link.getAttribute('data-key');
      const cleanKey = key.startsWith('/') ? key.slice(1) : key;
      fetch(`/accounts/mark-viewed/${cleanKey}/`, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } }).catch(() => {});
      fetch(`/accounts/record-download/${cleanKey}/`, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } }).catch(() => {});
    });
  });
}
...
async function showFavouritesModal() {
  try {
    const res = await fetch('/accounts/my-favourites/');
    const data = await res.json();
    showModal('My favourites', data.favourites || []);
  } catch (error) { console.error('Could not load favourites:', error); }
}

async function showBookmarksModal() {
  try {
    const res = await fetch('/accounts/my-bookmarks/');
    const data = await res.json();
    showModal('My bookmarks', data.bookmarks || []);
  } catch (error) { console.error('Could not load bookmarks:', error); }
}

async function showRecentModal() {
  try {
    const res = await fetch('/accounts/my-recent/');
    const data = await res.json();
    showModal('Recently viewed', data.recent || []);
  } catch (error) { console.error('Could not load recent items:', error); }
}
...
// ============================================
// BOOK READER — client-side with proxy fallback
// ============================================
...
async function fetchWithProxy(url) {
    const res = await fetch(`/accounts/fetch-book-text/?url=${encodeURIComponent(url)}`);
...
async function openReader(bookKey) {
  ...
  try {
    ...
    for (const [key, url] of Object.entries(formats)) {
      if (key.includes('text/plain') && key.toLowerCase().includes('utf-8')) { textUrl = url; break; }
    }
    if (!textUrl) {
      for (const [key, url] of Object.entries(formats)) {
        if (key.includes('text/plain')) { textUrl = url; break; }
      }
    }
    ...
    content.innerHTML = '<p style="text-align:center; color:#7d8aa3; padding:40px 0;">Fetching book content...</p>';

    let bookText = await fetchWithProxy(textUrl);
    ...
    for (const marker of startMarkers) {
      ...
      if (idx !== -1) { bookText = bookText.slice(bookText.indexOf('\n', idx) + 1); break; }
    }
    for (const marker of endMarkers) {
      const idx = bookText.indexOf(marker);
      if (idx !== -1) { bookText = bookText.slice(0, idx); break; }
    }
    ...
    for (let i = 0; i < paragraphs.length; i += 8) {
      bookPages.push(paragraphs.slice(i, i + 8).join('\n\n'));
    }
    ...
    content.innerHTML = '<p style="text-align:center; color:#ff6b6b; padding:40px 0;">Could not load book. Please try again in a moment.</p>';
...
function renderPage(page) {
  ...
  document.getElementById('reader-progress-bar').style.width = `${(currentPage / totalPages) * 100}%`;
...
async function loadRatings() {
  ...
  try {
    const res = await fetch(`/accounts/book-ratings/${cleanKey}/`);
    const data = await res.json();
...
async function submitRating() {
  ...
  try {
    const res = await fetch(`/accounts/rate-book/${cleanKey}/`, { method: 'POST', body: formData });
    const data = await res.json();
...
async function summarizeBook(bookKey) {
  ...
  try {
    const res = await fetch(`/accounts/summarize/${cleanKey}/`);
    const data = await res.json();
