      ...
      randomBooks.forEach(book => {
        ...
        bookItem.innerHTML = `
         ...
         <a href="https://openlibrary.org${key}" target="_blank" class="view-book-link download-btn" data-key="${key}">
          ...
          <button onclick="openReader('${key}', '${title.replace(/'/g, "\\'")}', '${author.replace(/'/g, "\\'")}'')" class="read-btn">📖 Read</button>
...
// ============================================
// IN-SITE BOOK READER — CLIENT SIDE (bypasses Django/Render network)
// ============================================
...
let currentPage = 1;
let totalPages = 1;
let bookPages = []; // all pages stored in browser memory

async function openReader(bookKey, title, author) {
  currentBookKey = bookKey;
  currentBookTitle = title || 'Unknown Title';
  currentBookAuthor = author || 'Unknown Author';
  currentPage = 1;
  bookPages = [];
  ...
  // Set title/author immediately
  const titleEl = document.getElementById('reader-title');
  const authorEl = document.getElementById('reader-author');
  if (titleEl) titleEl.textContent = currentBookTitle;
  if (authorEl) authorEl.textContent = currentBookAuthor;

  await fetchBookFromGutendex(title);
}

async function fetchBookFromGutendex(title) {
  const content = document.getElementById('reader-content');

  try {
    // Step 1: Search Gutendex by title
    const searchResp = await fetch(`https://gutendex.com/books/?search=${encodeURIComponent(title)}`);
    const searchData = await searchResp.json();
    const results = searchData.results || [];

    if (results.length === 0) {
      content.innerHTML = '<p style="text-align:center; color:#ff6b6b; padding:40px 0;">This book is not available for in-site reading. Try the download link instead.</p>';
      return;
    }

    // Step 2: Find text URL
    ...
    // Step 3: Fetch the actual book text
    content.innerHTML = '<p style="text-align:center; color:#7d8aa3; padding:40px 0;">Downloading book text...</p>';
    const textResp = await fetch(textUrl);
    let bookText = await textResp.text();

    if (!bookText.trim()) {
      content.innerHTML = '<p style="text-align:center; color:#ff6b6b; padding:40px 0;">Book content is empty. Try again.</p>';
      return;
    }

    // Step 4: Strip Gutenberg boilerplate
    ...
    for (const marker of startMarkers) {
      ...
      if (idx !== -1) {
        const endOfLine = bookText.indexOf('\n', idx);
        bookText = bookText.slice(endOfLine + 1);
        break;
      }
    }
    ...
    for (let i = 0; i < paragraphs.length; i += paragraphsPerPage) {
      bookPages.push(paragraphs.slice(i, i + paragraphsPerPage).join('\n\n'));
    }

    if (bookPages.length === 0) bookPages = ['No content available.'];

    totalPages = bookPages.length;
    currentPage = 1;

    // Step 6: Record view in Django (fire and forget)
    ...
    content.innerHTML = `<p style="text-align:center; color:#ff6b6b; padding:40px 0;">Could not load book: ${error.message}</p>`;
  }
}
...
function renderPage(page) {
  ...
  const pageContent = bookPages[currentPage - 1];
  ...
  if (watermark) watermark.textContent = watermark.textContent || '';
